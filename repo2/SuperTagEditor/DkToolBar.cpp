...
void CDkToolBar::SaveState()
{
    ...
    for (i=0;i<pluginToolBarInfo.GetSize();i++) {
        ...
        pIniFile->WriteInt(_T("STEP\\ToolbarPosition"), button->strRegName, nPos);
    }
    for (i=0;i<pluginToolBarInfo.GetSize();i++) {
        CPluginToolBarInfo* button = (CPluginToolBarInfo*)pluginToolBarInfo.GetAt(i);
        int nPos = GetToolBarCtrl().CommandToIndex(button->tbButton.idCommand);
    ...
    for (i=0;i<GetToolBarCtrl().GetButtonCount();i++) {
        ...
        if (GetToolBarCtrl().GetButton(i, &tbButton)) {
            pIniFile->WriteInt(_T("STEP\\DefaultToolbarPosition"), strRegName, tbButton.idCommand);
        }
    }
    {
        CString strRegName;
        strRegName.Format(_T("Pos%02d"), i);
        pIniFile->WriteInt(_T("STEP\\DefaultToolbarPosition"), strRegName, -1);
    }
    pIniFile->Flush();
}
...
void CDkToolBar::RestoreState()
{
    ...
    for (i=0;;i++) {
        CString strRegName;
        strRegName.Format(_T("Pos%02d"), i);
        int nID = pIniFile->ReadInt(_T("STEP\\DefaultToolbarPosition"), strRegName, -1);
...
void CDkToolBar::OnToolBarGetButtonInfo(NMHDR *notify, LRESULT *result)
{
    ...
    if (0 <= tbStruct->iItem && tbStruct->iItem < NButtons())
    {
        ...
        _tcsncpy_s(tbStruct->pszText, tbStruct->cchText, toolBarInfo[tbStruct->iItem].btnText, _TRUNCATE);
    ...
    else
    {
        if ((static_cast<long long>(tbStruct->iItem) - NButtons()) < pluginToolBarInfo.GetSize()) {
            ...
            if (OnToolTipNotify(lpButton->tbButton.idCommand, szText)) {
                _tcsncpy_s(tbStruct->pszText, tbStruct->cchText, szText, _TRUNCATE);
                *result = TRUE;
            } else {
                *result = FALSE;
            }
...
BOOL CDkToolBar::InsertButton(int nIndex, LPTBBUTTON lpButton, TCHAR *lpszRegName)
{
...
//  GetToolBarCtrl().InsertButton(nIndex, lpButton);
    CIniFile *pIniFile = ((CSuperTagEditorApp*)AfxGetApp())->GetIniFile();
    button->nPos = pIniFile->ReadInt(_T("STEP\\ToolbarPosition"), lpszRegName, -1);
//  if (nPos != -1) {
