...
static tagInfo STagInfo[] = {
	{_T("body"), BEGIN_BODY, END_BODY},
	{_T("center"), BEGIN_CENTER, END_CENTER},
	{_T("left"), BEGIN_LEFTJUST, END_LEFTJUST},
	{_T("right"), BEGIN_RIGHTJUST, END_RIGHTJUST},
	{_T("font"), BEGIN_FONT, END_FONT},
	{_T("b"), BEGIN_BOLD, END_BOLD},
	{_T("u"), BEGIN_UNDERLINE, END_UNDERLINE},
	{_T("i"), BEGIN_ITALICS, END_ITALICS},
	{_T("br"), NEWLINE, NEWLINE},
	{_T("h1"), HEADER1, TAG_UNKNOWN},
	{_T("h2"), HEADER2, TAG_UNKNOWN},
	{_T("h3"), HEADER3, TAG_UNKNOWN},
	{_T("h4"), HEADER4, TAG_UNKNOWN},
	{_T("h5"), HEADER5, TAG_UNKNOWN},
	{_T("h"), TAG_UNKNOWN, END_HEADER},
	{NULL, 0, 0}
};
...
static LPCTSTR GetString(LPCTSTR str, CString &output)
{
	...
	*/
	...
	TCHAR *newstr = output.GetBuffer(length + 1);
	LPCTSTR src = start;
	TCHAR *dst = newstr;
...
static LPCTSTR GetID(LPCTSTR src, CString &tagString, BOOL &isString)
{
	...
	if('"' == *src) {
		...
		length = end - src;
		TCHAR *tmp = tagString.GetBuffer(length + 1);
...
CString CSMLDoc::CSMLTagOptionList::GetParamTypeface(CString &defaultFace) {
	...
	if(NULL != value) {
		...
		if(0 == _tcsicmp(faceStr, _T("ANSI_FIXED_FONT"))) {
			id = ANSI_FIXED_FONT;
		} else if(0 == _tcsicmp(faceStr, _T("ANSI_VAR_FONT"))) {
			id = ANSI_VAR_FONT;
		} else if(0 == _tcsicmp(faceStr, _T("DEFAULT_GUI_FONT"))) {
			id = DEFAULT_GUI_FONT;
		} else if(0 == _tcsicmp(faceStr, _T("OEM_FIXED_FONT"))) {
			id = OEM_FIXED_FONT;
		} else if(0 == _tcsicmp(faceStr, _T("SYSTEM_FONT"))) {
			id = SYSTEM_FONT;
		} else if(0 == _tcsicmp(faceStr, _T("SYSTEM_FIXED_FONT"))) {
...
static BOOL GetHexNibble(TCHAR ch, UCHAR *out)
{
	BOOL retval;
...
static COLORREF ConvertColor(COLORREF defaultColor, const TCHAR *srcTxt)
{
	if(0 == _tcsnicmp(srcTxt, _T("COLOR_"), 6)) {
		srcTxt = &(srcTxt[6]);
		if(0 == _tcsicmp(srcTxt, _T("3DDKSHADOW"))) {
			defaultColor = COLOR_3DDKSHADOW;
		} else if(0 == _tcsicmp(srcTxt, _T("3DFACE"))) {
			defaultColor = COLOR_3DFACE;
		} else if(0 == _tcsicmp(srcTxt, _T("BTNFACE"))) {
			defaultColor = COLOR_BTNFACE;
		} else if(0 == _tcsicmp(srcTxt, _T("3DHILIGHT"))) {
			defaultColor = COLOR_3DHILIGHT;
		} else if(0 == _tcsicmp(srcTxt, _T("3DLIGHT"))) {
			defaultColor = COLOR_3DLIGHT;
		} else if(0 == _tcsicmp(srcTxt, _T("3DSHADOW"))) {
			defaultColor = COLOR_3DSHADOW;
		} else if(0 == _tcsicmp(srcTxt, _T("ACTIVEBORDER"))) {
			defaultColor = COLOR_ACTIVEBORDER;
		} else if(0 == _tcsicmp(srcTxt, _T("ACTIVECAPTION"))) {
			defaultColor = COLOR_ACTIVECAPTION;
		} else if(0 == _tcsicmp(srcTxt, _T("APPWORKSPACE"))) {
			defaultColor = COLOR_APPWORKSPACE;
		} else if(0 == _tcsicmp(srcTxt, _T("BACKGROUND"))) {
			defaultColor = COLOR_BACKGROUND;
		} else if(0 == _tcsicmp(srcTxt, _T("BTNTEXT"))) {
			defaultColor = COLOR_BTNTEXT;
		} else if(0 == _tcsicmp(srcTxt, _T("CAPTIONTEXT"))) {
			defaultColor = COLOR_CAPTIONTEXT;
#ifdef COLOR_GRADIENTACTIVECAPTION
		} else if(0 == _tcsicmp(srcTxt, _T("GRADIENTACTIVECAPTION"))) {
			defaultColor = COLOR_GRADIENTACTIVECAPTION;
#endif
#ifdef GRADIENTINACTIVECAPTION
		} else if(0 == _tcsicmp(srcTxt, _T("GRADIENTINACTIVECAPTION"))) {
			defaultColor = COLOR_GRADIENTINACTIVECAPTION;
#endif
		} else if(0 == _tcsicmp(srcTxt, _T("GRAYTEXT"))) {
			defaultColor = COLOR_GRAYTEXT;
		} else if(0 == _tcsicmp(srcTxt, _T("HIGHLIGHT"))) {
			defaultColor = COLOR_HIGHLIGHT;
		} else if(0 == _tcsicmp(srcTxt, _T("HIGHLIGHTTEXT"))) {
			defaultColor = COLOR_HIGHLIGHTTEXT;
#ifdef HOTLIGHT
		} else if(0 == _tcsicmp(srcTxt, _T("HOTLIGHT"))) {
			defaultColor = COLOR_HOTLIGHT;
#endif
		} else if(0 == _tcsicmp(srcTxt, _T("INACTIVEBORDER"))) {
			defaultColor = COLOR_INACTIVEBORDER;
		} else if(0 == _tcsicmp(srcTxt, _T("INACTIVECAPTION"))) {
			defaultColor = COLOR_INACTIVECAPTION;
		} else if(0 == _tcsicmp(srcTxt, _T("INACTIVECAPTIONTEXT"))) {
			defaultColor = COLOR_INACTIVECAPTIONTEXT;
		} else if(0 == _tcsicmp(srcTxt, _T("INFOBK"))) {
			defaultColor = COLOR_INFOBK;
		} else if(0 == _tcsicmp(srcTxt, _T("INFOTEXT"))) {
			defaultColor = COLOR_INFOTEXT;
		} else if(0 == _tcsicmp(srcTxt, _T("MENU"))) {
			defaultColor = COLOR_MENU;
		} else if(0 == _tcsicmp(srcTxt, _T("MENUTEXT"))) {
			defaultColor = COLOR_MENUTEXT;
		} else if(0 == _tcsicmp(srcTxt, _T("SCROLLBAR"))) {
			defaultColor = COLOR_SCROLLBAR;
		} else if(0 == _tcsicmp(srcTxt, _T("WINDOW"))) {
			defaultColor = COLOR_WINDOW;
		} else if(0 == _tcsicmp(srcTxt, _T("WINDOWFRAME"))) {
			defaultColor = COLOR_WINDOWFRAME;
		} else if(0 == _tcsicmp(srcTxt, _T("WINDOWTEXT"))) {
...
CSMLDoc::CSMLFontInfo::CSMLFontInfo(DWORD size, DWORD style, COLORREF color, LPCTSTR face)
{
	m_Size = size;
	m_Style = style;
	m_Color = color;
...
//
///////////////////////////////////////////////////////////////////////////////
CSMLDoc::CSMLTextGroup::CSMLTextBlock::CSMLTextBlock(LPCTSTR text, CSMLDoc::CSMLFontInfo *info)
{
	ASSERT(NULL != text);
	ASSERT(NULL != info);
...
BOOL CSMLDoc::ParseSMLText(UINT nID)
{
	...
	// Find the resource
	hSrc = FindResource(hInst, MAKEINTRESOURCE(nID), _T("SMLDOC"));
	if(NULL == hSrc) {
		return FALSE;
	}
	...
	while('\0' != *src) {
		...
		default:
			...
			if(true == inBody && length > 0) {
				TCHAR *txtBuf = new TCHAR[length + 1];
				TCHAR *dst = txtBuf;
