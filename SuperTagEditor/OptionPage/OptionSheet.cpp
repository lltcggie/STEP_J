...
BOOL COptionSheet::COptionList::NotifyParentOfSelChange()
{
	...
	// Send the message
	return wnd->SendMessage(WM_NOTIFY, hdr.idFrom, (LPARAM)&hdr);
}
...
BOOL COptionSheet::COptionList::NotifyParentOfSelChanging()
{
	...
	// Send the message
	return (BOOL)wnd->SendMessage(WM_NOTIFY, hdr.idFrom, (LPARAM)&hdr);
}
...
void COptionSheet::SetTitle(LPCTSTR lpszText, UINT nStyle)
{
	...
	if(nStyle & OSH_OPTIONTITLE) {
		m_strCaption = CString(_T("Options for "));
	}
...
void COptionSheet::AddButton(TCHAR *txt, int x, int w, UINT id, BOOL showButton)
{
	CButton *btn;
	CRect rect;
	CRect rcClient;
...
int COptionSheet::AddButtons(BOOL addBut)
{
	...
	if(TRUE == IsWizardMode()) {
		...
		// Add the back button
		if(TRUE == addBut) {
			AddButton(_T("< Back"), x, m_ButtonWidth, ID_WIZBACK);
		}
		x += m_ButtonWidth;

		// Add the next button
		if(TRUE == addBut) {
			AddButton(_T("Next >"), x, m_ButtonWidth, ID_WIZNEXT);
		}
		...
		if(m_psh.dwFlags & OSH_WIZARDHASFINISH) {
			...
			if(TRUE == addBut) {
				AddButton(_T("Finish"), x, m_ButtonWidth, ID_WIZFINISH);
			}
		} else {
			if(TRUE == addBut) {
				AddButton(_T("Finish"), x, m_ButtonWidth, ID_WIZFINISH, FALSE);
			}
		}
		...
		// Add the OK button
		if(TRUE == addBut) {
			AddButton(_T("OK"), x, m_ButtonWidth, IDOK);
		}
		x += m_ButtonWidth;
		...
		if(!(m_psh.dwFlags & OSH_NOAPPLYNOW)) {
			if(TRUE == addBut) {
				AddButton(_T("Apply"), x, m_ButtonWidth, ID_APPLY_NOW);
			}
			x += m_ButtonWidth;
		}
	...
	x += BUTTON_SPACING;
	if(TRUE == addBut) {
		AddButton(_T("Cancel"), x, m_ButtonWidth, IDCANCEL);
	}
	x += m_ButtonWidth;
	...
	if(m_psh.dwFlags & OSH_HASHELP) {
		x += BUTTON_SPACING;
		if(TRUE == addBut) {
			AddButton(_T("Help"), x, m_ButtonWidth, IDHELP);
		}
		x += m_ButtonWidth;
	}
	return x;
